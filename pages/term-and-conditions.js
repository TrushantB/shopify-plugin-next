    ...
    return (
        <div className="bg-indigo-100">
            ...
            <div className="max-w-5xl mx-auto text-center">
                <p className="w-full block text-center text-4xl font-bold my-3">Quick tips</p>
                {
                    termsAndConditions.map((item) => (
                        <div key={item.id} className="block  p-6 bg-white border border-gray-200 rounded-2xl shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700 my-3">
                            <p className="font-normal text-xl text-gray-700 dark:text-gray-400">{item.title}</p>
                        </div>
                    ))
                }
                ...
                <div className="flex items-center justify-center my-3">
                    <div onClick={submitTermsAndConditions} className={`rounded-full py-3 px-10 text-center font-bold text-white text-xl ${isAgreeTermsAndConditions ? 'cursor-pointer bg-indigo-500' : 'bg-gray-300'}`}>Customize Now</div>
                </div>
            </div>
            <Footer />
        </div>
